...
export class AppUtilBaseService {
  ...
  public formatCurrency(value: number, currencyCode: string, currencyDisplay?: string, currency?: string, locale?: string, col?: any): any {
    locale = locale || BaseAppConstants.defaultLocale;
    currencyCode = currencyCode || BaseAppConstants.currency;
    const currencySymbol = getCurrencySymbol(currencyCode, 'narrow', locale);
    if (col) {
  ...
  public getFormattedNumber(data: any, col: any): any {
    const minInteger = col.minInteger || BaseAppConstants.minInteger || 0;
    const maxFraction = col.maxFraction || BaseAppConstants.maxFraction || 0;
    const minFraction = col.minFraction || BaseAppConstants.minFraction || 0;
    const tS = col?.numberFormat?.thousandSeparator || BaseAppConstants.numberFormat?.thousandSeparator || ','
    const dS = col?.numberFormat?.decimalSeperator || BaseAppConstants.numberFormat?.decimalSeperator || '.'
  ...
  public getHourFormat(dateTimeFormat: string) {
    return dateTimeFormat?.toLowerCase()?.includes('a') ? '12' : (BaseAppConstants.dateTimeFormatPrimeNG?.toLowerCase()?.includes('a') ? '12' : '24')
  }
