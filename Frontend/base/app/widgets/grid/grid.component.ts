...
export class GridComponent implements OnInit {
  ...
  private getColumns(): any {
    ...
    if (this.gridConfig?.toggleColumns || this.gridConfig?.colReorder || this.gridConfig?.colResize || true) {
      this.gridConfig.fixedColumns.right = this.gridConfig.fixedColumns.right + 1
      columns.push({
        data: null,
  ...
  setVirtualDatatableOverflowScrollHeight() {
    ...
    if ((rowCountHeight ? (rowCountHeight * 31) : rowCountHeight) < (window.innerHeight - 250)) {
      $('.dataTables_scrollBody').css('max-height', (rowCountHeight * 31))
    } else {
