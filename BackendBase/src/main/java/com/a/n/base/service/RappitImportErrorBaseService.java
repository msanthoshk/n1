...
public class RappitImportErrorBaseService<T extends RappitImportErrorBase> extends BaseBusinessLogic<T>
		implements IRappitImportErrorBaseService<T> {
	...
	public RappitImportErrorBaseService(Class<T> modelClass) {
		super(modelClass);
	...
	protected void isObjectExists(T modelObj, boolean isUpdate) {
		...
		filters.add(new SimpleFilter("importId", modelObj.getImportId()));
		values.add(modelObj.getImportId());
		filters.add(new SimpleFilter("rowNumber", modelObj.getRowNumber()));
		values.add(modelObj.getRowNumber());
		...
		if(isUpdate) {
			...
			throw new AlreadyExistsException(ErrorCode.ALREADY_EXISTS, "RappitImportError", StringUtils.join(values,Constants.COMMA));
		}else {
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("RappitImportError", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("RappitImportError", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("RappitImportError", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
